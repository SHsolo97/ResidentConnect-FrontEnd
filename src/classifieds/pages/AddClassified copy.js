...
export const AddClassified1 = () => {
  ...
  const setClassifiedEmail = (event) => {
    const newEmails = classified.emails;
    newEmails[0] = event.target.value
    setClassified((prevState) => {
      return { ...prevState, emails: newEmails }
    });
  }
  ...
  const setPhoneHours = (data) => {
    const newPhone = classified.phone;
    newPhone[0].hours = data
    setClassified((prevState) => {
      return { ...prevState, phone: newPhone }
    });
  }


const setPhoneType = (data) => {
  const newPhone = classified.phone;
  newPhone[0].type = data
  setClassified((prevState) => {
    return { ...prevState, phone: newPhone }
  });
  ...
  <>
    ...
    <form>
      <Grid container direction="column" justifyContent="space-around" alignItems="flex-start">
        ...
        <TextField id="classifiedemail" style={{ margin: 8, width: '100ch' }} margin="normal" label="Classified Email" value={classified.emails[0]} onChange={setClassifiedEmail} variant="outlined" />
        <PhoneItem phone={classified.phone[0]} setPhoneNumber={setPhoneNumber} setPhoneType={setPhoneType} setPhoneHours={setPhoneHours} />
